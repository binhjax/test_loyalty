...
func (fw *F5WalletHandler)  creditHistory(conn *RpcConnection) ([]string, error) {
  ret := []string{}
...
func  (fw *F5WalletHandler) debitHistory(conn *RpcConnection) ([]string,error) {
  ret := []string{}
  instance, err := f5coin.NewBusiness(fw.ContractAddress,conn.Client)
...
func  (fw *F5WalletHandler) transferHistory(conn *RpcConnection) ([]string,error) {
  ret := []string{}
...
func  (fw *F5WalletHandler) stashNames(conn *RpcConnection) ([]string,error) {
  ret := []string{}
...
func  (fw *F5WalletHandler) getSummary(conn *RpcConnection) (int16,*big.Int, *big.Int, *big.Int,*big.Int, error)   {
      instance, err := f5coin.NewBusiness(fw.ContractAddress,conn.Client)
...
func (fw *F5WalletHandler) getBalance(conn *RpcConnection, stashName string) (*big.Int, error)  {
...
func (fw *F5WalletHandler) getState(conn *RpcConnection, stashName string) (int8, error)  {
  conn.Mux.Lock()
  defer  conn.Mux.Unlock()
...
func (fw *F5WalletHandler) getTransferHistoryLength(conn *RpcConnection) (*big.Int, error)  {
  conn.Mux.Lock()
  defer  conn.Mux.Unlock()
  session,err := f5coin.NewBusiness(fw.ContractAddress,conn.Client)
  if err != nil {
...
func (fw *F5WalletHandler) registerAccETH(conn *RpcConnection, auth *bind.TransactOpts, listAcc []common.Address) (*types.Transaction, error) {
      session,err := f5coin.NewBusiness(fw.ContractAddress,conn.Client)
      if err != nil {
          fmt.Println("Cannot find F5 contract")
          return nil,err
